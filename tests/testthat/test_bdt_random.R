#Automated testing for BDT
library(psychTestR)
library(testthat)

dir <- system.file("", package = "BDT", mustWork = TRUE)
...
BDT_ability_sem <<- results[["BDT"]][["ability_sem"]]
BDT_ability <<- results[["BDT"]][["ability"]]

print(paste("Standard error of measurement of BDT", BDT_ability_sem))
app$stop()
